...
learns a latent variable model for its input data. So instead of letting your 
neural network learn an arbitrary function, you are learning the parameters of 
a probability distribution modeling your data. If you sample points from this 
distribution, you can generate new input data samples: a VAE is a "generative 
model". Please refer to the research papers by [Kingma et al.][vae_ref1] and 
[Rezende et al.][vae_ref2] for a thorough mathematical analysis.

More formally, VAEs represent latent space (bottleneck layer) as a Guassian 
random variable (enabled by a constraint on the loss function). In the 
probability model framework, a variational autoencoder contains a specific 
probability model of data $x$ and latent variables $z$. We can write the joint 
...
In terms of VAE, the latent variables are generated by the encoder and
...
through random sampling and that is the problem because backpropogation cannot 
flow through random node; to overcome this obstacle,
...
distribution. We approximate $p(z)$ with normally distributed
$\mathcal{N}(0, 1)$.

$$p(z) \sim \mu + L \mathcal{N}(0, 1)$$

Here $\mu$ and $L$ are the output of the encoder. Therefore while backpropogation, all we need is partial derivatives w.r.t. $\mu$, $L$.

You can read more about VAE [here][vae1], [here][vae2], [here][vae3] and [here][vae].
...
We can low load our training data and train the model.
